...
\begin{document}
  ...
  %Einband
    ...
    \vspace*{\fill}
    \begin{Huge}
      { \color{kjgtuerkis}  Stand November 2024}
    \end{Huge}
    \vspace*{\fill}
    \newpage
  %Zitat Thoms Morus
...
\chapter{Allgemeine Regelungen zur Satzung}

\section{Geschlechterdefinitionen innerhalb der Katholischen jungen Gemeinde}\label{geschlechterdefinition}

Geschlechtergerecht im Rahmen dieser Satzung bedeutet: Gremien (und Ämter) werden mit männlichen und
weiblichen Personen paritätisch besetzt.
Bei Gremien mit einer Größe von 2 Personen werden 2 Stellen für Personen unterschiedlicher Geschlechterkategorie eingerichtet.
Bei Gremien mit einer Größe von bis zu 10 Personen wird zusätzlich
eine, bei mehr als 10 Personen zwei Stellen für INTA* Personen eingerichtet.

Die folgenden Geschlechterkategorien finden in der KjG Anwendung:


\begin{itemize}
    \item Weiblich im Rahmen dieser Satzung bezeichnet Personen, die sich als tendenziell weiblich
          identifizieren, z.B. cis, trans* und inter* Frauen.
    \item Männlich im Rahmen dieser Satzung bezeichnet Personen, die sich als tendenziell männlich
          identifizieren, z.B. cis, trans* und inter* Männer.
    \item INTA* im Rahmen dieser Satzung bezeichnet Personen, die sich als nicht oder nicht nur weiblich
          und nicht oder nicht nur männlich identifizieren oder genderfluid sind. 
          INTA* steht dabei für inter*, nichtbinär, trans* und agender und weitere Geschlechtskategorien 
          außerhalb des binären Systems. 
          Dies erweitert den bisher verwendeten Begriff divers.
\end{itemize}


\section{Befähigung zur Geistlichen Leitung} \label{geistlicheleitung}
...
\section{Beschränkte Geschäftsfähigkeit} \label{geschaeftsfaehigkeit}
...
Um eine Pfarr- bzw. Ortsgruppe aufzulösen muss ein 
Auflösungsprozess nach Anlage \glqq Auflösung einer Pfarr- oder Ortsgruppe\grqq{} der 
Bundessatzung durchgeführt werden.
...
\begin{itemize}
  ...
  \item Sorge für die Mitgliedergewinnung und -pflege auf Pfarrebene
        sowie Meldung der Mitglieder an die jeweilig zuständigen Stellen
\end{itemize}

\subsubsection{Zusammensetzung der Pfarrleitung}

Die Pfarrleitung ist geschlechtergerecht (\ref{geschlechterdefinition}) zu
besetzen, ihr gehören mindestens an:

Stimmberechtigt:
\begin{itemize}
  \item 5 Pfarrleiter*innen (2 männlich, 2 weiblich, 1 INTA*)
  \item 2 Geistliche Leiter*innen (\ref{geistlicheleitung}) unterschiedlicher Geschlechterkategorie
\end{itemize}
Die Aufgaben der Pfarrleitung können auch wahrgenommen werden, wenn nicht alle Stellen besetzt sind.
Von der Verpflichtung zur geschlechtergerechten (\ref{geschlechterdefinition})
Besetzung sind die KjG Pfarrgemeinschaften ausgenommen, in
denen nur Personen einer Geschlechterkategorie vertreten sind.

Mindestens ein Mitglied der Pfarrleitung muss voll geschäftsfähig sein.\\
Für mindestens die Hälfte der Stellen müssen beschränkt geschäftsfähige Personen (\ref{geschaeftsfaehigkeit})
...
Der Kindersenat ist geschlechtergerecht (\ref{geschlechterdefinition}) zu besetzen,
...
Von der Verpflichtung zur geschlechtergerechten (\ref{geschlechterdefinition}) Besetzung sind die KjG Pfarrgemeinschaften ausgenommen,
in denen nur Personen einer Geschlechterkategorie vertreten sind.
...
\begin{itemize}
  ...
  \item Die Wahl einer geschlechtergerecht (\ref{geschlechterdefinition}) zu besetzenden Leitung
\end{itemize}
...
Die Diözesankonferenz kann für bestimmte Aufgaben geschlechtergerecht 
(\ref{geschlechterdefinition}) besetzte Sachausschüsse einrichten.
Sachausschüsse zu geschlechterkategoriespezifischen Belangen sind hiervon ausgenommen.

Den Vorsitz der Sachausschüsse hat ein Mitglied der Diözesanleitung inne, dieser kann delegiert
werden.

Der Wahlausschuss leitet die Wahlen. Der Wahlausschuss ist geschlechtergerecht (\ref{geschlechterdefinition}) zu besetzen. Den Vorsitz
...
\begin{itemize}
  \item{ 2 Mitglieder der Pfarrleitung bzw. von Pfarrleitung oder Mitgliederversammlung
        Delegierte unterschiedlicher Geschlechterkategorien}
\end{itemize}

Von der Verpflichtung zur geschlechtergerechten (\ref{geschlechterdefinition}) Besetzung
...
Stimmrecht. Der Diözesanausschuss kann im Einzelfall von dieser
Regelung abweichen und einer KjG Pfarrgemeinschaft, deren Stimmrecht ruht, ein
Stimmrecht gewähren.
...
\begin{itemize}
  ...
  \item 1 INTA* Mitglied der Pfarrleitungen bzw. Mitglied einer
...
Personen unterschiedlicher Geschlechterkategorie Geistliche Leitung
 (\ref{geistlicheleitung}).
...
Mitglied im Diözesanausschuss können Personen werden, die mindestens beschränkt geschäftsfähig (\ref{geschaeftsfaehigkeit}) sind.
Von den stimmberechtigten Mitgliedern des Diözesanausschusses, die nicht Teil der Diözesanleitung sind,
muss aber mindestens eine Person, unabhängig der Geschlechterkategorie, voll geschäftsfähig sein.
...
\begin{itemize}
  ...
  \item Sorge für die Mitgliedergewinnung und –pflege auf Diözesanebene sowie
        Meldung der Mitglieder an die KjG-Bundesebene
  \item Beratung und Unterstützung der Bezirksebene sowie der Pfarrebene
        in der Mitgliedergewinnung und -pflege
\end{itemize}
...
\begin{itemize}
	...
	\item 1 INTA* Mitglied.
\end{itemize}

Von diesen Personen sind bis zu zwei Personen unterschiedlicher 
Geschlechterkategorie Geistliche Leitung (\ref{geistlicheleitung}).
...
Männer und INTA* Delegierte nur durch INTA* Personen vertreten werden. Die Vereinigung mehrerer Stimmen auf eine Person ist unzulässig.
...
Unterlagen. Diese beinhalten mindestens:
\begin{itemize}
  \item die vorläufige Tagesordnung
  \item die Anträge mit Begründung
\end{itemize}

Bei mindestens einer Diözesankonferenz pro Kalenderjahr müssen die Unterlagen
zusätzlich folgendes beinhalten:
\begin{itemize}
  \item den Bericht der Diözesanleitung
  \item den Bericht des Diözesanauschusses
  \item die Berichte über die Arbeit der diözesanen Teams und Arbeitsgruppen
\end{itemize}
...
der stimmberechtigten Mitglieder anwesend sind sowie keine Geschlechterkategorie mehr als 75 Prozent der anwesenden stimmberechtigten Mitglieder ausmacht.
...
Es werden nach Geschlechterkategorie getrennte Redner*innenlisten geführt. Diese Listen werden im Wechsel aufgerufen.
...
Das Wort wird durch die*den Vorsitzende*n in der Reihenfolge des Eingangs der Wortmeldungen erteilt.
Es werden nach Geschlechterkategorie getrennte Redner*innenlisten geführt. Diese Listen werden im Wechsel aufgerufen. Berichte werden
